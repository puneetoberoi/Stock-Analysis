...
def generate_enhanced_html_email(df_stocks, context, market_news, macro_data, memory, portfolio_data, pattern_data, ai_analysis, portfolio_recommendations=None):
    ...
    if portfolio_data and 'stocks' in portfolio_data:
        ...
        for stock in portfolio_data['stocks']:
            ...
            if stock.get('bollinger') and stock['bollinger'].get('squeeze'):
                bb_indicator = f"<br><span style='color:#f59e0b;font-weight:bold;'>💥 SQUEEZE</span>"
            if stock.get('week52') and 'AT 52W HIGH' in stock['week52'].get('signal', ''):
    ...
    if portfolio_data and portfolio_data.get('learning_active'):
        ...
        if predictions_made > 0:
                    ...
                    <div style="border: 1px solid #ddd; border-left: 5px solid {action_color}; border-radius: 8px; margin-bottom: 20px; padding: 20px; background: #fff;">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <div>
                                ...
                                <p style="margin: 0; color: #777; font-size: 1em;">{stock.get('name', '')}</p>
                            </div>
                        ...
                        <div style="margin: 20px 0; padding-top: 20px; border-top: 1px solid #eee;">
                            ...
                            {breakdown_list or "<p style='font-size:0.9em; color:#777;'>No detailed breakdown available.</p>"}
                        </div>
            ...
            if prediction_cards:
                ai_predictions_html = f"""<div class="section" style="background-color:#f4f7f6;"><h2 class="section-title">🎯 AI Predictions & Conviction Analysis</h2>{''.join(prediction_cards)}</div>"""
                logging.info(f"✅ AI predictions HTML generated successfully with {len(prediction_cards)} cards.")
            else:
                logging.warning("⚠️ No prediction cards were created, though predictions were expected.")
        else:
    ...
    if pattern_data and pattern_data.get('matches'):
        ...
        current_cond = f"""<div style="background-color:#fff;padding:15px;border:2px solid #7c3aed;border-radius:5px;margin-bottom:20px;">...</div>""" # Truncated
        interpretation_html = ""
        ...
        # This is where your old recommendations_html was, now removed.
        pattern_html = f"""<div class="section" style="background-color:#f3e8ff;border-left:4px solid #7c3aed;">...</div>""" # Truncated

    # Editor's note and final data prep
    editor_note = "Good morning..." # Truncated
    sector_html = "..." # Truncated
    ...
    <div class="container">
        ...
        <div class="section"><h2>SECTOR DEEP DIVE</h2>{sector_html or "<p><i>No data available.</i></p>"}</div>
        <div class="section">
            <h2>STOCK RADAR</h2>
            <h3>📈 Top 10 Strongest Signals</h3><table><thead><tr><th>Company</th><th>Score</th></tr></thead><tbody>{top10_html}</tbody></table>
            <h3 style="margin-top: 30px;">📉 Top 10 Weakest Signals</h3><table><thead><tr><th>Company</th><th>Score</th></tr></thead><tbody>{bottom10_html}</tbody></table>
        </div>
        <div class="section">
            <h2>BEYOND STOCKS: Alternative Assets</h2>
            <h3>🪙 Crypto</h3><p><b>Market Sentiment: {context.get('crypto_sentiment', 'N/A') if context else 'N/A'}</b></p><table><thead><tr><th>Asset</th><th>Price / 24h</th><th>Market Cap</th></tr></thead><tbody>{crypto_html}</tbody></table>
            <h3 style="margin-top: 30px;">💎 Commodities</h3><p><b>Gold/Silver Ratio: {context.get('gold_silver_ratio', 'N/A') if context else 'N/A'}</b></p><table><thead><tr><th>Asset</th><th>Price / 24h</th><th>Market Cap</th></tr></thead><tbody>{commodities_html}</tbody></table>
        </div>
